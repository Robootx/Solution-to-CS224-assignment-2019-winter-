...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=self.model_embeddings.embed_size,
                                hidden_size=self.hidden_size,
                                bias=True,
                                bidirectional=True)
        self.decoder = nn.LSTMCell(input_size=(self.model_embeddings.embed_size+self.hidden_size),
                                    hidden_size=self.hidden_size,
                                    bias=True)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # Set e_t to -inf where enc_masks has 1('pad' tokens)
    ...
    def beam_search_(self, src_sent:List[str], beam_size:int=5, max_decoding_time_step:int=70) -> List[Hypothesis]:
        ...
        src_encodings_att_linear = self.att_projection(src_encodings) # (1, src_len_len, h)
        h_tm1 = dec_init_vec
        att_tm1 = torch.zeros(1, self.hidden_size, device=self.device) # O_0: (1, h)
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # (hyp_num, src_len, 2h)
            exp_src_encodings = src_encoding.expand(hyp_num,
                                                    src_encoding.size(1),
                                                    src_encoding.size(2))
            # (hyp_num, src_len, h)
            ...
            y_t_embed = self.model_embeddings.target(y_tm1) # (hyp_num, e)
            x = torch.cat([y_t_embed, att_tm1], dim=-1) # Ybar_t: (hyp_num, e+h)
            (h_t, cell_t), att_t, _  = self.step(Ybar_t=x, 
                                                dec_state=h_tm1,
                                                enc_hiddens=enc_exp_src_encodings, 
                                                enc_hiddens_proj=exp_src_encodings_att_linear, 
                                                enc_masks=None)
            # log probabilities over target words
            log_p_t = F.log_softmax(self.target_vocab_projection(att_t), dim=-1) # (hyp_num, V)
